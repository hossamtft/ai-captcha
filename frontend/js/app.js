...
async function loadTemporal() {
    ...
    // Position the zone based on server data
...
function animateTemporal() {
    ...
    // Use speed_segments for accurate animation (matches server calculation)
    ...
    // Activate zone highlight when indicator is within the green zone
    const zone = document.getElementById('temporal-zone');
...
async function loadBehavioural() {
    ...
    state.behavioural = {
        ...
        isActive: false,
        trajectory: [],
        visited: new Array(c.waypoints.length).fill(false),
        startTime: 0,
        timerInterval: null,
        nonce: c.nonce  // Store nonce for replay attack prevention
    };
